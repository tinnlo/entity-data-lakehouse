...
shell environment cannot accidentally trigger ClickHouse or PostgreSQL
mutations or an unexpected ML backend variant.

Overrides applied for every test in this directory:
- ``USE_CLICKHOUSE`` → ``"false"``   (prevents real ClickHouse connections)
- ``USE_POSTGRES``   → ``"false"``   (prevents real PostgreSQL connections)
